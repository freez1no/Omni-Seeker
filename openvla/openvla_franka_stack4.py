...
def run_openvla_demo():
    ...
    for step in range(10):
        ...
        # predict_action 사용 (taco_play로 변경)
        with torch.no_grad():
            action_vector = model.predict_action(**inputs, unnorm_key="taco_play", do_sample=False)
        
        
        
        # ===== 스케일링 추가 =====
        POSITION_SCALE = 0.1  # 위치 변화를 1/10로 축소
        ROTATION_SCALE = 0.2  # 회전 변화를 1/5로 축소
