...
def run_openvla_demo():
    ...
    # ===============================
    # OpenVLA의 action_vector는 delta 값이므로 현재 pose에 더해야 함
    current_ee_position, current_ee_rotation = art_kinematics_solver.compute_end_effector_pose()

    # Delta를 적용 (OpenVLA의 출력은 상대적 변화)
    target_position = current_ee_position + action_vector[:3]

    # Roll, pitch, yaw를 quaternion으로 변환
    # OpenVLA는 delta roll/pitch/yaw를 출력하므로 현재 orientation에 더해야 함
    from scipy.spatial.transform import Rotation as R

    # 현재 rotation을 euler angle로 변환
    ...
    if not success:
        print("[WARN] IK solution not found! Using current position.")
    else:
